...
export async function copyTemplateFiles(targetDirPath: string, fileName: string, includeStorybook: Boolean) {
	const componentDir: String = targetDirPath.split('/')[1];
	const sourceDirPath: fs.PathLike = path.join('./src/templates');
	let cssClassName = '';
	let storybookTitle = '';
	...
	switch (componentDir) {
		...
		default: {
			...
			switch (fileName.charAt(0)) {
				case 'C':
					storybookTitle = `${capitalizeFirstLetter(componentDir)}/Content ${capitalizeFirstLetter(
						componentDir
					)}/${fileName}`;
					break;
				case 'T':
					storybookTitle = `${capitalizeFirstLetter(componentDir)}/Teaser ${capitalizeFirstLetter(
						componentDir
					)}/${fileName}`;
					break;
				case 'L':
					storybookTitle = `${capitalizeFirstLetter(componentDir)}/List ${capitalizeFirstLetter(
						componentDir
					)}/${fileName}`;
					break;
				case 'E':
					storybookTitle = `${capitalizeFirstLetter(componentDir)}/Extra ${capitalizeFirstLetter(
						componentDir
					)}/${fileName}`;
					break;
			}
